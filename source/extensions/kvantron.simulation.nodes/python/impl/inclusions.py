...
# TODO: Don't use Replicator for temporary objects.
# Replicator isn't supposed for it.
# Instead, use a workflow similar to the uncoated areas one.
class InclusionsGenerator:
    def __init__(self):
        ext_manager = omni.kit.app.get_app().get_extension_manager()
        ext_path = Path(ext_manager.get_extension_path_by_module("kvantron.simulation.nodes"))
        textures_folder = ext_path.joinpath(
            "data", "optical_sorting_stage", "Assets", "Textures"
        )

        self.inclusion_texture = textures_folder.joinpath("Inclusion_BaseColor.png").as_posix()

    def _get_node_type(self, node: og.Node):
    ...
    def _get_nodes_to_delete(
        ...
        ogn_groups = (n for n in replicator_graph.get_nodes() if self._get_node_type(n) == "omni.replicator.core.OgnGroup")
        ...
        nodes_to_upstream_traverse = (n for n in replicator_graph.get_nodes() if self._get_node_type(n) in ("omni.replicator.core.OgnWritePrimAttribute", "omni.replicator.core.OgnScatter2D"))
    ...
    def _get_prims_to_delete(self, projector_prims: rep.utils.ReplicatorItem) -> list[Sdf.Path]:
    ...
    def vanish_inclusions_defect(
        ...
        nodes_to_delete = self._get_nodes_to_delete(stage, projector_prims, modify_projection_material)
        prims_to_delete = self._get_prims_to_delete(projector_prims)
        ...
        for node in nodes_to_delete:
            ...
            if self._get_node_type(node) == "omni.graph.action.OnImpulseEvent":
    ...
    def create_inclusions_defect(
        ...
        projector_prims = rep.create.cube(
            ...
            rotation=(0, -90, 0),
            scale=rep.distribution.uniform(0.002, 0.0035)
        )
        ...
        with projection_materials:
            modify_projection_material = rep.modify.projection_material(
                diffuse=self.inclusion_texture#Path(r"C:\Users\Alexey\optical-sorting-app\source\extensions\kvantron.simulation.nodes\data\optical_sorting_stage\Assets\Textures\Inclusion_BaseColor.png").as_posix(),
            )
