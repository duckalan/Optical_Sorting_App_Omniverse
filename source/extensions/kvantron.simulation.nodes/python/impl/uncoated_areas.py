...
noises_folder = Path(ext_path).joinpath(
    "data", "optical_sorting_stage", "Assets", "Textures", "UncoatedAreas"
)

noise_textures = []
for file in os.listdir(noises_folder.as_posix()):
    noise_textures.append(noises_folder.joinpath(file).as_posix())

def create_uncoated_areas(cap_prim: Usd.Prim):
    ...
    texture_input.Set(choice(noise_textures))